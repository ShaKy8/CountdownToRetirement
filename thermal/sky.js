/**
 * VENDORED - the live sky, adapted for THERMAL.
 *
 * Source:          weather/js/gl/sky.js
 * Upstream commit: f31edca
 * Upstream sha256: 908f79cdd041dd1558301465e00a53c2ef245d2e0f5c4e6d4971f0975c4b0a50
 * Vendored:        2026-09-07
 *
 * weather/ is generated output: scripts/sync-weather.sh does `rm -rf` and
 * re-copies the whole tree from ../Weather/public. Importing it from here would
 * break SILENTLY the next time the shader is iterated on upstream - the file
 * would still load, the class would still construct, render() would still draw,
 * and the glider would simply be in the wrong part of a differently-composed
 * sky. Nothing would throw. A test warns when the hash above goes stale.
 *
 * Local edits from upstream, kept exhaustive:
 *   1. ES export -> the repo's IIFE dual-export shim, so this is a classic
 *      script like flight.js and tests.js can require() it.
 *   2. New uniform `uSkyline` (0..1) gating the whole synthetic city: the
 *      skyline geometry, the neon perspective ground grid below the horizon,
 *      and the glow spilling up off the roofline. Upstream draws all three
 *      unconditionally, which is wrong over open country. It defaults to 1 so
 *      upstream behaviour is unchanged, and THERMAL passes 0.
 *   3. The shader's screen geometry is exported as HORIZON_Y / EL_PER_NDC /
 *      AZ_PER_NDC, because the 2D terrain layer has to agree with it and a
 *      silent disagreement is a detached horizon.
 */
(function (root) {
    'use strict';
    ...
    uniform float uSkyline;      // 0 = open country, 1 = the console's neon city
    ...
    void main() {
      ...
      float cityH = skyline(az) * uSkyline;
      ...
      if (el < cityH) {
        ...
        if (el >= 0.0) {
          ...
          ground = vec3(0.004, 0.008, 0.020) + gridCol * grid * fade * 0.42 * uSkyline;
      ...
      // Glow spilling upward off the skyline into the sky.
      {
        ...
        if (above > 0.0) {
          col += (mix(vec3(0.9, 0.45, 0.15), vec3(1.0, 0.25, 0.60), nightAmt)
                 ...
                 * mix(gloom, 1.0, nightAmt * 0.7)) * uSkyline;
        }
      }
    ...
    const DEFAULTS = {
      ...
      kp: 2, lat: 33.6, haze: 0.15, cityGlow: 0.5, skyline: 1,
    };
    ...
    class Sky {
      ...
      init() {
        ...
        for (const n of ['uRes', 'uTime', 'uQuality', 'uSunAlt', 'uSunAz', 'uMoonAlt', 'uMoonAz',
          ...
          'uSnow', 'uStorm', 'uFlash', 'uFog', 'uWind', 'uWindDir', 'uKp', 'uLat', 'uHaze', 'uCityGlow', 'uSkyline']) {
      ...
      render(nowMs, dt) {
        ...
        gl.uniform1f(u.uSkyline, p.skyline);
    ...
    const ThermalSky = {
        Sky: Sky,
        DEFAULTS: DEFAULTS,
        // The shader's screen geometry, exported from the file whose GLSL
        // defines it: el = (uv.y - HORIZON_Y) * EL_PER_NDC and
        // az = uViewAz + uv.x * aspect * AZ_PER_NDC. The terrain layer projects
        // with these, so if the shader moves and these do not, the ridge line
        // quietly detaches from the horizon. A test asserts they agree.
        HORIZON_Y: -0.30,
        EL_PER_NDC: 0.95 * Math.PI * 0.42,
        AZ_PER_NDC: 0.95
    };

    if (typeof module !== 'undefined' && module.exports) {
        module.exports = ThermalSky;
    }
    if (root) {
        root.ThermalSky = ThermalSky;
    }
